import { useRef, useEffect, useState } from "react";
import { useGLTF, useAnimations, OrbitControls } from "@react-three/drei";
import {useFrame, useThree} from '@react-three/fiber'
import {a} from '@react-spring/three'

import islandScene from '../assets/3d/B&WPlanet.glb'

const Island = ({isRotating, setIsRotating, setCurrentStage, islandPos,...props}) => {
    const islandRef = useRef();

    const {gl, viewport} = useThree();


    const group = useRef();
    const { nodes, materials, animations } = useGLTF(islandScene);
    const { actions } = useAnimations(animations, group);

    const [floatingPosition, setFloatingPosition] = useState([0, 0, 0]);
    const floatingAmplitude = 0.5; // Adjust the floating height
    const floatingSpeed = 0.002; // Adjust the floating speed

    useEffect(() => {
        // Update the island's position for a floating effect outside of useFrame
        const updateFloatingPosition = () => {
            const yPos = Math.sin(Date.now() * floatingSpeed) * floatingAmplitude;
            setFloatingPosition((prev) => [prev[0], yPos, prev[2]]);
        };

        const updateId = setInterval(updateFloatingPosition, 16); // Update every frame
        return () => clearInterval(updateId);
    }, []);

    useFrame(() => {
        // Update the island's position using the state
        islandRef.current.position.set(...floatingPosition);
    });

    return (
        <a.group ref={islandRef} {...props}>
            <group position={[0.182, 0.02, -0.026]}>
                <mesh
                    castShadow
                    receiveShadow
                    geometry={nodes.Icosphere_1.geometry}
                    material={materials["Material.003"]}
                />
                <mesh
                    castShadow
                    receiveShadow
                    geometry={nodes.Icosphere_2.geometry}
                    material={materials["Material.011"]}
                />
            </group>
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Icosphere001.geometry}
                material={materials["Material.004"]}
                position={[0.182, 0.02, -0.026]}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane.geometry}
                material={materials["Material.005"]}
                position={[0.182, 0.02, -0.026]}
                rotation={[-0.631, 0.095, -0.29]}
                scale={-0.069}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane002.geometry}
                material={materials["Material.005"]}
                position={[0.403, 0.342, -0.367]}
                rotation={[-0.747, 0.137, -0.404]}
                scale={-0.105}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane004.geometry}
                material={materials["Material.005"]}
                position={[0.031, -0.05, 0.012]}
                rotation={[-0.868, 0.166, -0.075]}
                scale={-0.062}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane005.geometry}
                material={materials["Material.005"]}
                position={[0.28, 0.15, -0.192]}
                rotation={[-0.879, 0.169, -0.351]}
                scale={-0.086}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane006.geometry}
                material={materials["Material.005"]}
                position={[0.277, 0.286, -0.303]}
                rotation={[-0.868, 0.166, -0.075]}
                scale={-0.093}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane007.geometry}
                material={materials["Material.005"]}
                position={[0.054, 0.339, -0.296]}
                rotation={[-0.849, 0.168, 0.231]}
                scale={-0.091}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane008.geometry}
                material={materials["Material.005"]}
                position={[0.135, 0.184, -0.439]}
                rotation={[-1.124, 0.117, 0.133]}
                scale={-0.096}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane009.geometry}
                material={materials["Material.005"]}
                position={[0.108, 0.365, -0.037]}
                rotation={[-0.112, 0.363, 0.315]}
                scale={-0.085}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane010.geometry}
                material={materials["Material.005"]}
                position={[0.041, 0.304, -0.057]}
                rotation={[-0.003, 0.402, 0.416]}
                scale={-0.081}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane011.geometry}
                material={materials["Material.005"]}
                position={[-0.107, 0.465, -0.317]}
                rotation={[-0.329, 0.234, 0.57]}
                scale={-0.097}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane012.geometry}
                material={materials["Material.005"]}
                position={[0.034, 0.499, -0.025]}
                rotation={[0.186, 0.464, 0.299]}
                scale={-0.094}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane013.geometry}
                material={materials["Material.005"]}
                position={[-0.061, 0.312, -0.184]}
                rotation={[-0.05, 0.36, 0.594]}
                scale={-0.083}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane014.geometry}
                material={materials["Material.005"]}
                position={[0.087, 0.173, -0.151]}
                rotation={[-0.197, 0.256, 0.677]}
                scale={-0.071}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane015.geometry}
                material={materials["Material.005"]}
                position={[0.066, 0.258, 0.081]}
                rotation={[0.499, 0.581, 0.229]}
                scale={-0.083}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane016.geometry}
                material={materials["Material.005"]}
                position={[0.172, 0.227, -0.032]}
                rotation={[0.433, 0.575, -0.037]}
                scale={-0.078}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane017.geometry}
                material={materials["Material.005"]}
                position={[0.196, 0.094, -0.084]}
                rotation={[0.29, 0.555, 0.373]}
                scale={-0.065}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane018.geometry}
                material={materials["Material.005"]}
                position={[0.188, -0.019, -0.206]}
                rotation={[0.338, 0.578, 0.652]}
                scale={-0.056}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane019.geometry}
                material={materials["Material.005"]}
                position={[-0.022, 0.489, -0.044]}
                rotation={[0.083, 0.401, 0.673]}
                scale={-0.093}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane020.geometry}
                material={materials["Material.005"]}
                position={[-0.03, 0.255, -0.077]}
                rotation={[0.125, 0.44, 0.922]}
                scale={-0.08}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane021.geometry}
                material={materials["Material.005"]}
                position={[0.143, 0.207, 0.581]}
                rotation={[1.748, 1.176, -0.004]}
                scale={-0.113}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane022.geometry}
                material={materials["Material.005"]}
                position={[0.074, 0.263, -0.111]}
                rotation={[1.298, 1.141, 0.381]}
                scale={-0.067}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane023.geometry}
                material={materials["Material.005"]}
                position={[-0.039, 0.337, 0.308]}
                rotation={[1.15, 1.116, 0.359]}
                scale={-0.098}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane024.geometry}
                material={materials["Material.005"]}
                position={[0.162, 0.311, 0.366]}
                rotation={[1.817, 1.116, -0.36]}
                scale={-0.102}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane025.geometry}
                material={materials["Material.005"]}
                position={[0.126, 0.218, 0.167]}
                rotation={[1.973, 1.09, -0.342]}
                scale={-0.087}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane026.geometry}
                material={materials["Material.005"]}
                position={[0.11, 0.165, -0.018]}
                rotation={[1.404, 1.136, -0.007]}
                scale={-0.075}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane027.geometry}
                material={materials["Material.005"]}
                position={[0.175, 0.138, -0.322]}
                rotation={[1.973, 1.09, -0.342]}
                scale={-0.056}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane028.geometry}
                material={materials["Material.005"]}
                position={[0.08, 0.313, 0.117]}
                rotation={[0.849, 1.009, 0.6]}
                scale={-0.082}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane029.geometry}
                material={materials["Material.005"]}
                position={[-0.035, 0.16, 0.095]}
                rotation={[0.8, 0.982, 0.947]}
                scale={-0.081}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane030.geometry}
                material={materials["Material.005"]}
                position={[0.086, 0.145, -0.1]}
                rotation={[2.022, 1.075, -0.692]}
                scale={-0.071}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane031.geometry}
                material={materials["Material.005"]}
                position={[-0.01, 0.236, -0.084]}
                rotation={[1.259, 1.204, -0.042]}
                scale={-0.074}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane032.geometry}
                material={materials["Material.005"]}
                position={[0.488, 0.35, -0.463]}
                rotation={[2.008, 0.215, -1.978]}
                scale={-0.11}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane033.geometry}
                material={materials["Material.005"]}
                position={[0.283, 0.271, -0.308]}
                rotation={[1.944, 0.057, -1.924]}
                scale={-0.093}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane034.geometry}
                material={materials["Material.005"]}
                position={[0.081, 0.122, -0.184]}
                rotation={[1.904, -0.015, -2.22]}
                scale={-0.077}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane035.geometry}
                material={materials["Material.005"]}
                position={[-0.123, -0.102, -0.612]}
                rotation={[1.176, -0.459, -2.991]}
                scale={-0.107}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane036.geometry}
                material={materials["Material.005"]}
                position={[0.404, -0.307, -1.209]}
                rotation={[1.275, -0.43, -2.67]}
                scale={-0.153}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane037.geometry}
                material={materials["Material.005"]}
                position={[-0.119, -0.517, -1.187]}
                rotation={[0.989, -0.509, -3.021]}
                scale={-0.153}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane038.geometry}
                material={materials["Material.005"]}
                position={[-0.134, 0.106, -0.048]}
                rotation={[-0.104, 0.036, 2.167]}
                scale={-0.081}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane039.geometry}
                material={materials["Material.005"]}
                position={[-0.271, 0.092, 0.02]}
                rotation={[-0.139, 0.099, 1.995]}
                scale={-0.091}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane040.geometry}
                material={materials["Material.005"]}
                position={[0.289, 0.27, -0.095]}
                rotation={[-0.15, 0.14, 1.673]}
                scale={-0.053}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane041.geometry}
                material={materials["Material.005"]}
                position={[-0.269, 0.151, -0.12]}
                rotation={[-0.126, -0.163, 1.63]}
                scale={-0.092}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane042.geometry}
                material={materials["Material.005"]}
                position={[-0.394, 0.126, 0.029]}
                rotation={[-0.147, -0.048, 1.876]}
                scale={-0.098}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane043.geometry}
                material={materials["Material.005"]}
                position={[-0.601, 0.163, -0.206]}
                rotation={[-0.081, -0.327, 1.722]}
                scale={-0.111}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane044.geometry}
                material={materials["Material.005"]}
                position={[-0.164, 0.329, -0.108]}
                rotation={[-0.076, 0.014, 1.387]}
                scale={-0.087}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane045.geometry}
                material={materials["Material.005"]}
                position={[-0.062, 0.304, -0.223]}
                rotation={[-0.396, -0.86, 1.085]}
                scale={-0.086}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane046.geometry}
                material={materials["Material.005"]}
                position={[0.548, -0.516, -0.07]}
                rotation={[1.782, -0.87, -1.093]}
                scale={-0.123}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane047.geometry}
                material={materials["Material.005"]}
                position={[0.196, -0.271, 0.013]}
                rotation={[2.155, -1.069, -0.406]}
                scale={-0.095}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane048.geometry}
                material={materials["Material.005"]}
                position={[0.194, -0.953, -0.033]}
                rotation={[2.66, -1.143, -0.219]}
                scale={-0.139}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane050.geometry}
                material={materials["Material.006"]}
                position={[0.182, 0.02, -0.026]}
                rotation={[2.292, -0.624, 0.206]}
                scale={[-0.064, -0.046, -0.046]}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane051.geometry}
                material={materials["Material.006"]}
                position={[0.024, 0.113, 0.351]}
                rotation={[-1.394, 1.258, -3.14]}
                scale={[-0.04, -0.029, -0.029]}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane052.geometry}
                material={materials["Material.006"]}
                position={[0.013, -0.063, 0.324]}
                rotation={[-2.964, -1.414, -1.476]}
                scale={[0.041, 0.029, 0.029]}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane053.geometry}
                material={materials["Material.006"]}
                position={[-0.179, 0.045, 0.31]}
                rotation={[-0.585, 1.159, -0.847]}
                scale={[0.039, 0.028, 0.028]}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane054.geometry}
                material={materials["Material.006"]}
                position={[0.048, -0.049, 0.24]}
                rotation={[0.285, -0.748, -1.278]}
                scale={[-0.054, -0.038, -0.038]}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane055.geometry}
                material={materials["Material.006"]}
                position={[-0.234, 0.036, -0.034]}
                rotation={[-1.978, -0.095, 2.212]}
                scale={[-0.053, -0.037, -0.037]}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane056.geometry}
                material={materials["Material.006"]}
                position={[-0.303, -0.245, -0.447]}
                rotation={[1.289, 0.354, 2.675]}
                scale={[-0.029, -0.02, -0.02]}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane057.geometry}
                material={materials["Material.006"]}
                position={[-0.268, -0.12, -0.533]}
                rotation={[-1.973, -0.038, -2.575]}
                scale={[0.028, 0.02, 0.02]}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane058.geometry}
                material={materials["Material.006"]}
                position={[-0.347, -0.012, -0.601]}
                rotation={[-1.758, -0.038, -2.525]}
                scale={[0.024, 0.017, 0.017]}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane059.geometry}
                material={materials["Material.006"]}
                position={[-0.295, -0.026, -0.602]}
                rotation={[-1.89, 0.426, -2.622]}
                scale={[0.026, 0.018, 0.018]}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane060.geometry}
                material={materials["Material.006"]}
                position={[-0.352, 0.02, -0.527]}
                rotation={[-2.052, 0.538, -2.192]}
                scale={[0.029, 0.021, 0.021]}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane061.geometry}
                material={materials["Material.006"]}
                position={[-0.358, 0.087, -0.651]}
                rotation={[-1.642, 0.335, -2.642]}
                scale={[0.022, 0.016, 0.016]}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane062.geometry}
                material={materials["Material.006"]}
                position={[-0.206, -0.307, -0.465]}
                rotation={[-2.854, 0.856, -2.494]}
                scale={[0.027, 0.019, 0.019]}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane063.geometry}
                material={materials["Material.006"]}
                position={[-0.371, -0.222, -0.382]}
                rotation={[-3.021, 0.748, -2.292]}
                scale={[0.028, 0.02, 0.02]}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane064.geometry}
                material={materials["Material.005"]}
                position={[0.099, 0.165, -0.068]}
                rotation={[2.364, 0.897, -0.81]}
                scale={-0.071}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane065.geometry}
                material={materials["Material.005"]}
                position={[0.142, 0.181, -0.136]}
                rotation={[2.324, 0.861, -1.089]}
                scale={-0.071}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane066.geometry}
                material={materials["Material.005"]}
                position={[0.09, 0.25, -0.189]}
                rotation={[2.247, 0.869, -1.341]}
                scale={-0.071}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane067.geometry}
                material={materials["Material.005"]}
                position={[0.064, 0.302, -0.151]}
                rotation={[1.755, 1.104, -0.869]}
                scale={-0.074}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane068.geometry}
                material={materials["Material.006"]}
                position={[0.307, 0.063, 0.136]}
                rotation={[-0.155, 0.166, -1.641]}
                scale={[-0.052, -0.037, -0.037]}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane069.geometry}
                material={materials["Material.006"]}
                position={[0.057, 0.039, -0.599]}
                rotation={[-1.696, -1.51, -3.093]}
                scale={[0.034, 0.024, 0.024]}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane070.geometry}
                material={materials["Material.006"]}
                position={[-0.361, 0.407, -0.234]}
                rotation={[2.139, -0.27, 2.45]}
                scale={[-0.037, -0.026, -0.026]}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane071.geometry}
                material={materials["Material.006"]}
                position={[-0.124, 0.531, -0.213]}
                rotation={[2.774, 0.923, -0.321]}
                scale={[0.037, 0.026, 0.026]}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane072.geometry}
                material={materials["Material.006"]}
                position={[0.377, 0.415, 0.015]}
                rotation={[2.846, 0.68, -2.411]}
                scale={[-0.037, -0.026, -0.026]}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane073.geometry}
                material={materials["Material.005"]}
                position={[0.237, 0.33, -0.044]}
                rotation={[0.027, -0.111, 2.434]}
                scale={-0.049}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane074.geometry}
                material={materials["Material.005"]}
                position={[-0.061, 0.191, -0.157]}
                rotation={[0.206, -0.286, 2.283]}
                scale={-0.073}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane075.geometry}
                material={materials["Material.005"]}
                position={[0.111, 0.309, 0.078]}
                rotation={[0.206, -0.286, 2.283]}
                scale={-0.055}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane076.geometry}
                material={materials["Material.005"]}
                position={[0.02, 0.256, -0.094]}
                rotation={[0.286, -0.361, 2.444]}
                scale={-0.064}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane077.geometry}
                material={materials["Material.006"]}
                position={[0.242, -0.275, -0.39]}
                rotation={[-2.589, -0.364, -0.205]}
                scale={[-0.032, -0.023, -0.023]}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane078.geometry}
                material={materials["Material.006"]}
                position={[0.241, -0.303, -0.124]}
                rotation={[-0.548, 0.827, 0.885]}
                scale={[0.036, 0.026, 0.026]}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Plane079.geometry}
                material={materials["Material.006"]}
                position={[0.164, -0.434, -0.21]}
                rotation={[2.784, -1.161, -0.723]}
                scale={[-0.03, -0.021, -0.021]}
            />
            <group
                position={[0.182, 0.02, -0.026]}
                rotation={[-0.26, 0.012, 0.91]}
                scale={-0.01}
            >
                <mesh
                    castShadow
                    receiveShadow
                    geometry={nodes.Plane004_1.geometry}
                    material={materials["Material.007"]}
                />
                <mesh
                    castShadow
                    receiveShadow
                    geometry={nodes.Plane004_2.geometry}
                    material={materials["Material.008"]}
                />
            </group>
            <group
                position={[-0.011, 0.044, -0.389]}
                rotation={[0.365, 0.081, -2.352]}
                scale={-0.013}
            >
                <mesh
                    castShadow
                    receiveShadow
                    geometry={nodes.Plane004_1.geometry}
                    material={materials["Material.007"]}
                />
                <mesh
                    castShadow
                    receiveShadow
                    geometry={nodes.Plane004_2.geometry}
                    material={materials["Material.008"]}
                />
            </group>
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Icosphere002.geometry}
                material={materials["Material.009"]}
                position={[0.182, 0.02, -0.026]}
                rotation={[0.558, 0.146, 0.743]}
                scale={0.08}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Icosphere003.geometry}
                material={materials["Material.010"]}
                position={[0.182, 0.02, -0.026]}
                rotation={[0.42, 0.959, -0.591]}
                scale={0.08}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Icosphere004.geometry}
                material={materials["Material.012"]}
                position={[0.182, 0.02, -0.026]}
                rotation={[0.144, 0.527, -2.779]}
                scale={0.08}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Icosphere005.geometry}
                material={materials["Material.009"]}
                position={[-0.122, 0.003, -0.089]}
                rotation={[0.326, -0.681, -1.243]}
                scale={0.08}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Icosphere006.geometry}
                material={materials["Material.009"]}
                position={[-0.131, 0.135, -0.154]}
                rotation={[0.953, -1.058, -1.631]}
                scale={0.08}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Icosphere007.geometry}
                material={materials["Material.010"]}
                position={[0.182, -0.039, -0.15]}
                rotation={[2.62, 0.125, 1.706]}
                scale={0.08}
            />
            <mesh
                castShadow
                receiveShadow
                geometry={nodes.Icosphere008.geometry}
                material={materials["Material.009"]}
                position={[-0.047, 0.223, 0.061]}
                rotation={[-1.69, 0, -0.949]}
                scale={0.08}
            />
        </a.group>
    );
}

useGLTF.preload("/NewIsland4.glb");

export default Island;